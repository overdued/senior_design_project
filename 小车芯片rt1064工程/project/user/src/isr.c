...
void LPUART1_IRQHandler(void)
{
    ...
    {
			uint8_t Rxdata = LPUART_ReadByte(LPUART1);//维特比陀螺仪
			jy901_receivedata(Rxdata);
...
void LPUART4_IRQHandler(void)
{
    ...
    {
        // 接收中断 
		  uart4_rx_interrupt_handler_1();//自己加入
...
void GPIO1_Combined_0_15_IRQHandler(void)
{
    ...
    if(exti_flag_get(B10))
    {
				
        exti_flag_clear(B10);// 清除中断标志位
				HC_SR04_handler();
    }
}
...
void GPIO3_Combined_0_15_IRQHandler(void)
{
    ...
    if(exti_flag_get(D4))
    {
        exti_flag_clear(D4);// 清除中断标志位
		    ...
		    if(exti_flag_get(D14))
    {
				
        exti_flag_clear(D14);// 清除中断标志位
				HC_SR04_2_handler();
    }
}

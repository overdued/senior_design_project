...
class TestGarbageIdentify:
    ...
    def setup_method(self):
        ...
        self.garbage_identify = garbage_identify(test_mode=True)
    ...
    def test_garbage_grap_green(self):
        res = self.garbage_identify.garbage_grap({'Fish_bone': (-0.0075, 0.21773)}, [89, 134])
        assert res is None
        
    # 测试红色垃圾时，机械臂的运动
    def test_garbage_grap_red(self):
        res = self.garbage_identify.garbage_grap({'Syringe': (-0.0075, 0.21773)}, [89, 134])
        assert res is None
        
    # 测试蓝色垃圾时，机械臂的运动
    def test_garbage_grap_blue(self):
        res = self.garbage_identify.garbage_grap({'Newspaper': (-0.0075, 0.21773)}, [89, 134])
        assert res is None
        
    # 测试灰色垃圾时，机械臂的运动
    def test_garbage_grap_gray(self):
        res = self.garbage_identify.garbage_grap({'Cigarette_butts': (-0.0075, 0.21773)}, [89, 134])
        assert res is None

...
const char *urdf_file = "/home/HwHiAiUser/E2ESamples/ros2_dofbot_formal_ws/src/dofbot_moveit/urdf/dofbot.urdf"; // REVISE
int a = 0;
...
int main(int argc, char ** argv)
{
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("dofbot_kinemarics_server");
  RCLCPP_INFO(node->get_logger(), "ROS Server Starts..");
//   rclcpp::sleep_for(std::chrono::milliseconds(3000));
//   RCLCPP_INFO(node->get_logger(), "ROS Waiting1..");
//   rclcpp::sleep_for(std::chrono::milliseconds(3000));
//   RCLCPP_INFO(node->get_logger(), "ROS Waiting2..");
//   rclcpp::sleep_for(std::chrono::milliseconds(3000));
//   RCLCPP_INFO(node->get_logger(), "ROS Waiting3..");
//   rclcpp::sleep_for(std::chrono::milliseconds(3000));
//   RCLCPP_INFO(node->get_logger(), "ROS Still Waiting..");
  rclcpp::Service<dofbot_info::srv::Kinemarics>::SharedPtr trial_service_ = node->create_service<dofbot_info::srv::Kinemarics>("trial_service", &srvicecallback);
  rclcpp::spin(node);
  rclcpp::shutdown();
}

...
class GarbageIdentify:
    def __init__(self, test_mode=False):
        ...
        model_path = os.path.join(model_folder, "yolov8x_32.om")
        label_path = os.path.join(model_folder, "classes.txt")
        cfg_folder = os.path.join(share_root, "config")
        offset_cfg_path = os.path.join(cfg_folder, "offset.txt")

        if test_mode:
            lib_root = os.path.dirname(FILE.parents[0])
            model_path = os.path.join(lib_root, "model", "yolov8x_32.om")
            label_path = os.path.join(lib_root, "model", "classes.txt")
    ...
    def garbage_grap(self, msg, xy,degree):
        """
        执行抓取函数
        :param msg: {name:pos,...}
        ...
        for elm in new_msg:
            try:
                ...
                # 调取移动函数
                self.grap_move.arm_run(str(name), joints,degree)
            except Exception:
                print("sqaure_pos empty")
        # # 初始位置
        joints_0 = [self.xy[0], self.xy[1], 0, 0, 135, 30]
    ...
    def garbage_run(self, image, garbage_index=None):
        ...
        try:
            ...
            self.node.get_logger().error("get_pos failed:\n%s" % traceback.format_exc())
        return self.frame, msg
            ...
            for _, det in enumerate(pred):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if prediction_status:
                        ...
                        self.last_detection_coords[name] = (x1, y1, x2, y2)
        return msg

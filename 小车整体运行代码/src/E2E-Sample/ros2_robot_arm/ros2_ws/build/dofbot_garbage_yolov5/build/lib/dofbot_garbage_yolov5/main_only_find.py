...
def wait_for_message(
    ...
    backlog: int = 1,
    max_read: int = 1024 * 1024,  #1MB 安全上限，避免恶意大
    ) -> Tuple[bytes, Tuple[str, int]]:
    
    #启动一次 ?TCP 监听，直到收到满 ?match 条件的数据后返回 ?    返回 (data, addr)。无 match 时，收到任意非空数据即返回 ?
    #- host: 监听地址，例 ?"0.0.0.0" 或具体网 ?"192.168.1.100"
    #- port: 端口
    #- match: 匹配函数，入参为 bytes，返 ?True 表示满足条件
    #- timeout: socket 超时（秒），包含 accept  ?recv
    #- backlog: listen 的排队长 ?    - max_read: 最大累计读取字节数（防御性）
...
def is_box_in_center_region(x1, y1, x2, y2, img_width=640, img_height=480, center_ratio=0.4):
    """
    检查检测框是否在图像中心区域附 ?    :param x1, y1, x2, y2: 检测框坐标 (像素)
    ...
    print(f"Box是否在中心区 ? {is_in_center}")
...
def search_and_grasp_target(target_object_name="Plastic bag"):
    ...
    max_angle = 120     # 最大角 ?    
    # 识别逻辑控制变量
    phase = 1  # 1: 90度测试阶 ? 2: 全角度遍历阶 ?    
    detection_results = []  # 存储检测结 ?[(angle, is_target_found, is_in_center), ...]
    max_detections_per_angle = 10  # 每个角度最大检测次 ?    
    target_detection_threshold = 1  # 需要成功检测到目标的次 ?    
    # 中心区域检测参 ?    
    ...
    while capture.isOpened() and not Grapfinishstate:
        ...
        if phase == 1:
            # 阶段1:  ?0度位置测 ?            
            print(f"阶段1:  ?0度位置进行测 ?(第{len([r for r in detection_results if r[0]==90])+1} ?")
                ...
                if len(msg) != 0:
                    ...
                    for name in object_names:
                        ...
                        if name.lower() == target_object_name.lower():
                            target_found = True
                            Finddegree = 90
                            print(f"找到目标物体 {target_object_name}，角 ?", Finddegree)
                            ...
                            if hasattr(target, 'last_detection_coords') and name in target.last_detection_coords:
                                ...
                                print(f"Box是否在中心区 ? {box_in_center}")
                            else:
                                box_in_center = True
            ...
            # 阶段2:  ?0度开始遍历所有角 ?          
            print(f"阶段2: 在{current_angle}度位置进行检")
            ...
            try:
                ...
                for name in msg.keys():
                    if hasattr(target, 'last_detection_coords') and name in target.last_detection_coords:
                        ...
                        cv.putText(img, name, (x1, y1 - 10), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

                # 实时显示画面
                cv.imshow("RGB View", img)
                ...
                print(f"{current_angle}度检测结 ? 成功{successful_detections} ? 总共{total_detections}")
                
                # 判断是否满足条件或达到最大检测次 ?               
                if total_detections >= max_detections_per_angle:
                    if successful_detections >= target_detection_threshold and target_found:
                        print(f"在{current_angle}度位置成功检测到目标 {target_object_name} 且在中心区域，执行抓 ?..")
                        target.garbage_grap(msg, xy, current_angle)
                        sleep(2)
                        hand_open()          # 张开机械手
                        sleep(0.5)
                        arm.Arm_serial_servo_write6_array(grab_position, 1000)  # 手臂到位
                        sleep(1)
                        hand_grasp()         # 抓取
                        sleep(1)
                        hand_default()       # 归位
                        ...
                        print(f"在{current_angle}度位置未满足抓取条件，切换到下一个角 ?..")
                        # 切换到下一个角 ?                        
                        current_angle += angle_step
                        if current_angle > max_angle:
                            print("已完成所有角度的搜索，未找到合适目标，重新开始新一轮搜 ?..")
                            # 开始下一轮搜 ?                            
                            current_round += 1
                            print(f"开始第 {current_round} 轮搜 ?..")
                            # 重置搜索参数，直接进入阶 ?（从30度开始）
                            ...
                            current_angle = min_angle
                            detection_results = []
                        detection_results = []  # 清空当前角度的检测结 ?                        
                # 如果检测到目标但没有抓取，继续寻找下一个角 ?                
                elif target_found and not box_in_center:
                    print(f"在{current_angle}度位置检测到目标但不在中心区域，切换到下一个角 ?..")
                    current_angle += angle_step
                    if current_angle > max_angle:
                        print("已完成所有角度的搜索，未找到合适目标，重新开始新一轮搜 ?..")
                        current_round += 1
                ...
                if len(current_angle_results) >= max_detections_per_angle:
                    print(f"在{current_angle}度位置检测次数已达上限，切换到下一个角 ?..")
                    current_angle += angle_step
                    if current_angle > max_angle:
                        print("已完成所有角度的搜索，未找到合适目标，重新开始新一轮搜 ?..")
                        current_round += 1
...
def Grap(text):
    search_and_grasp_target(text)  # 抓取塑料 ?   
    cv.destroyAllWindows()
    return
...
# ========== 配置 ==========
MODEL = "/root/ascend-devkit/src/E2E-Sample/ros2_robot_arm/ros2_ws/src/dofbot_garbage_yolov5/dofbot_garbage_yolov5/yolov8n_310b.om"
...
#/root/ascend-devkit/src/E2E-Sample/ros2_robot_arm/ros2_ws/src/dofbot_garbage_yolov5/dofbot_garbage_yolov5/classes.txt
YOLO_INPUT_SIZE = 640
...
# 类别ID
TARGET_CLASS_IDS_OBJECT = {345,62}   # 要抓的物 ?
TARGET_CLASS_IDS_BEGIN  = {381}   # 原点标志 ?
# 相机参数
...
def main():
    ...
    # ========== 固定目标 ==========
    TARGET_CLASS_IDS_OBJECT = {62}   # Box
    target_arm = "Box"

    print("🚀 开始识别：检测到目标后直接抓取（无深 ?/ 无小车）")
    ...
    try:
        while True:
            ...
            print(f"[INFO] 推理耗时 {infer_ms:.1f} ms，检测到 {len(dets)} 个目")
            ...
            for det in dets:
                *xyxy, conf, cls = det
                ...
                if class_id not in TARGET_CLASS_IDS_OBJECT:
                    continue

                print(f"🎯 识别到目 ?class_id={class_id}, conf={conf:.2f}")
                found = True
                break

            if found:
                print("🧹 释放 YOLO 和摄像头资源")
                cap.release()

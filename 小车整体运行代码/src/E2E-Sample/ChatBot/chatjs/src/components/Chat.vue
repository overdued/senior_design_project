...
// 录音
let recoder = ref<any>(null)
let isRecoder = ref<boolean>(false)
...
function stopRecoder(){
    ...
    recoder.value.destroy()
    .then(()=>{
        recoder.value = null
    })
    isRecoder.value = false
...
function startOrStop(){
	...
	if(isRecoder.value){
        ...
        recoder.value = new Recoder(param)
		recoder.value.start()
        isRecoder.value = true
	}
}
...
function chatToUser() {
    ...
    if(originMessage.value){
        ...
        sendDisable.value = true
		isSend.value = true
		const url = `/chat?message=${originMessage.value}`
		fetch(url)
		.then(res=>res.json())
		.then(res=>{
			if(res.code === 200){
...
.chatBox::-webkit-scrollbar { width: 0!important }
.chatBottom{
    display: flex;
    flex-direction: column;
}

 ...
 *
 * 2022.08.25-Add class ImageWriter function parameters
...
int Cli::main()
{
    ...
    {
        _imageWriter->setSrc(args[0], 2 ,0, 0, parser.value(sha256Option).toLatin1() );
    }
    else
    {
        QFileInfo fi(args[0]);
        ...
        if (fi.isFile())
        {
            _imageWriter->setSrc(QUrl::fromLocalFile(args[0]), 2 , fi.size(), 0, parser.value(sha256Option).toLatin1() );
        }
        else if (!fi.exists())
        {

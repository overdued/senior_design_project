 ...
 *
 * 2022.08.25-Changed organization name and icon
...
//    QCoreApplication::setAttribute(Qt::AA_UseHighDpiPixmaps);
...
#endif
    app.setOrganizationName("Huawei Technologies Co., Ltd.");
    app.setOrganizationDomain("ascend.com");
    app.setApplicationName("Ascend AI Devkit Imager");
    app.setWindowIcon(QIcon(":/icons/logo_red.svg"));
    ...
    if (customQm.isEmpty())
    {
        ...
        if (translator->load(QLocale(), "rpi-imager", "_", QLatin1String(":/i18n")))
        {
            ...
            imageWriter.sendLanguage(translator->language());
        }
        else
            delete translator;
    }
    ...
    engine.rootContext()->setContextProperty("localurl", "");
    ...
    if (!url.isEmpty()) {
        imageWriter.setSrc(url, 2);
        //如果url不为空 qml界面本地镜像烧录显示对应的地址
        engine.rootContext()->setContextProperty("localurl", url);
    }
    ...
    qmlwindow->connect(&imageWriter, SIGNAL(info(QVariant)), qmlwindow, SLOT(onInfo(QVariant)));
    ...
    qmlwindow->connect(&imageWriter, SIGNAL(downUpdateexeProgress(QVariant,QVariant)), qmlwindow, SLOT(onDownUpdateexeProgress(QVariant,QVariant)));

...
DownloadExtractThread::DownloadExtractThread(const QByteArray &url, const QByteArray &localfilename, const QByteArray &expectedHash, bool isExtensible, QObject *parent)
    : DownloadThread(url, localfilename, expectedHash, isExtensible, parent), _abufsize(IMAGEWRITER_BLOCKSIZE), _ethreadStarted(false),

 ...
 *
 * 2022.08.25-Add set disk signature function and move RSV function
...
std::atomic<bool> DownloadThread::_checkRemoteFile{false};
std::atomic<bool> DownloadThread::_compliantRemoteFile{true};
QByteArray DownloadThread::_proxy;
int DownloadThread::_curlCount = 0;

char _decToHex(int num)
{
    if (num > 9)
    {
        return ('A' + num - 10);
...
DownloadThread::DownloadThread(const QByteArray &url, const QByteArray &localfilename, const QByteArray &expectedHash, bool isExtensible, QObject *parent) :
    ...
    _inputBufferSize(0), _file(NULL), _writehash(OSLIST_HASH_ALGORITHM), _verifyhash(OSLIST_HASH_ALGORITHM), _isExtensible(isExtensible)
{
    if (!_curlCount)
    ...
    DownloadThread::_checkRemoteFile = true;
    DownloadThread::_compliantRemoteFile = true;
}
...
size_t DownloadThread::_curl_header_callback( void *ptr, size_t size, size_t nmemb, void *userdata)
{
    ...
    int pos = headerstr.find("Content-Length: ");
    if(pos != std::string::npos && DownloadThread::_checkRemoteFile)
    {
        DownloadThread::_checkRemoteFile = false;
        std::string sub = headerstr.substr(16);
        long long num = std::atoll(sub.c_str());
        if (num < IMAGEWRITER_REMOTE_FILE_MINIMUM_SIZE)
        {
            qDebug() << headerstr.c_str();
            DownloadThread::_compliantRemoteFile = false;
            return 0;
        }
    }
    return len;
...
QByteArray DownloadThread::_getRandomSign()
{
    std::mt19937 mt_rand(std::chrono::high_resolution_clock::now().time_since_epoch().count());
    unsigned int rand = mt_rand();
    QByteArray result;
    result += _decToHex((rand & 0xF0000000) >> 28);
    result += _decToHex((rand & 0x0F000000) >> 24);
    result += _decToHex((rand & 0x00F00000) >> 20);
    result += _decToHex((rand & 0x000F0000) >> 16);
    result += _decToHex((rand & 0x0000F000) >> 12);
    result += _decToHex((rand & 0x00000F00) >> 8);
    result += _decToHex((rand & 0x000000F0) >> 4);
    result += _decToHex((rand & 0x0000000F));
    return result;
}

bool DownloadThread::_callDiskpart(const QByteArray &data)
{
    qDebug() << "diskpartCmd : " << data;
    QProcess proc;
    proc.start("diskpart");
    proc.waitForStarted();
    proc.write(data);
...
bool DownloadThread::_openAndPrepareDevice()
{
    ...
    if (std::regex_match(_filename.constData(), m, windriveregex))
    {
        ...
        if (!_nr.isEmpty()) {
            ...
            QByteArray diskpartCmd = "select disk "+_nr+"\r\n"
                                     "clean\r\n"
                                     "rescan\r\n";
            if (!_callDiskpart(diskpartCmd))
            {
                return false;
            }
        }
    ...
    if (authopenresult == _file.authOpenCancelled) {
        ...
        msg += "<br>"+tr("Please verify if 'Imager' is allowed access to 'removable volumes' in privacy settings (under 'files and folders' or alternatively give it 'full disk access').");
...
#ifdef Q_OS_WIN
            if (!DownloadThread::_compliantRemoteFile)
            {
                _onDownloadError(tr("Error remote file less than 1MB."));
            }
            else if (_file.errorCode() == ERROR_ACCESS_DENIED)
            {
...
//    }
    ...
    // 移动RSV区域到末尾
    if (_bytesWritten < _fileSize && _isExtensible)
    {
         qDebug() << "Start Moving SectorRSV!";
        if (_moveSectorRSV.MovingSectorRSV(_file, _bytesWritten, _fileSize, _sectorSize) != 0)
        {
            DownloadThread::_onDownloadError(tr("Error moving sectorRSV failed."));
            _closeFiles();
            return;
        }
    }
    ...
    //重新设置磁盘签名
    QByteArray diskSign = _getRandomSign();
    QByteArray diskpartCmd = "select disk "+_nr+"\r\n"
                             "uniqueid disk ID="+diskSign+"\r\n";
    if (!_callDiskpart(diskpartCmd))
    {
        return;
    }

...
int ImageWriter::getOperation()
{
    return static_cast<int>(_operateType);
}

/* Set URL to download from */
void ImageWriter::setSrc(const QUrl &url, int operateType, quint64 downloadLen, quint64 extrLen, QByteArray expectedHash,
                         bool multifilesinzip, QString parentcategory, QString osname, QByteArray initFormat, bool isExtensible)
{
    _src = url;
    _downloadLen = downloadLen;
    _operateType = static_cast<OperationType>(operateType);
    _extrLen = extrLen;
...
void ImageWriter::sendLanguage(QString msg)
{
    // 中文环境用中文的json文件 其他环境都用英文
    if(msg == "zh_CN")
        _repo = QUrl(QString(OSLIST_URL));
    else
        _repo = QUrl(QString(OSLIST_URL_EN));
}

void ImageWriter::httpDownLoad(const QString &url)
{
    QUrl downUrl=QUrl::fromUserInput(url);
    qDebug() << "url"<<url;
    qDebug() << "downUrl.fileName()"<< downUrl.fileName();
    QString fullFileName="./"+downUrl.fileName();
    ...
    reply=networkManager.get(QNetworkRequest(downUrl));
    //关联信号与槽
...
void ImageWriter::onReadyRead()
{
    //读取下载的数据
    downloadedFile->write(reply->readAll());
}

void ImageWriter::onDownloadProgress(qint64 bytesRead, qint64 totalBytes)
{
    //设置进度条的值
    emit downUpdateexeProgress(bytesRead, totalBytes);
}

//网络请求结束之后
void ImageWriter::onFinished()
{
    QFileInfo fileInfo;
    fileInfo.setFile(downloadedFile->fileName());
    QString exeName = downloadedFile->fileName();
    //关闭打开的网络文件，然后置空
    ...
    qDebug() << exeName <<"下载更新包成功";
    QProcess::startDetached(exeName);
    exit(0);
}

bool ImageWriter::fileIsExist(QString filename)
{
...
bool ImageWriter::writeIpInfo(QString filePath, QString ipData)
{
    QFile f(filePath);
    if(f.exists()&&f.open(f.ReadWrite | f.Truncate))
    {
        f.write(ipData.toUtf8());
        f.close();
        return true;
    }
    return false;
...
void ImageWriter::waitTime(qint16 time)
{
    QThread::sleep(time);
}
...
void ImageWriter::startWrite()
{
    ...
    if (_operateType == OperationType::FORMAT)
    {
    ...
    if (_operateType == OperationType::READBACK)
    {
        _extrLen = 0;
        QByteArray urlstr = _src.toString(_src.FullyEncoded).mid(9).toLatin1();
        _thread = new DriveBackupThread(urlstr, _dst.toLatin1(), _readbackCompression, this);
    ...
    {
        _thread = new LocalFileExtractThread(urlstr, _dst.toLatin1(), _expectedHash, _isExtensible, this);
    }
    else if (compressed)
    {
        _thread = new DownloadExtractThread(urlstr, _dst.toLatin1(), _expectedHash, _isExtensible, this);
        // 屏蔽对下载镜像信息进行上报到服务器功能
...
//        }
    ...
    {
        _thread = new DownloadThread(urlstr, _dst.toLatin1(), _expectedHash, _isExtensible, this);
    ...
    if (!_expectedHash.isEmpty() && _cachedFileHash != _expectedHash && _cachingEnabled)
    {
        ...
        if (_cachingEnabled)
        {
            ...
            if (avail < _downloadLen || avail-_downloadLen < IMAGEWRITER_MINIMAL_SPACE_FOR_CACHING)
            {
                qDebug() << "Low disk space. Not caching files to disk.";
                emit info(tr("Low disk space. Not caching files to disk."));
            }
            else
            {
...
void ImageWriter::_parseCompressedFile()
{
    ...
    QByteArray fn = _src.toLocalFile().toLocal8Bit();
    int numFiles = 0;
    _extrLen = 0;
...
void ImageWriter::setReadbackCompression(bool isCompressed)
{
    _readbackCompression = isCompressed;
}

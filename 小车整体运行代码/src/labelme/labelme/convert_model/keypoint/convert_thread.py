...
class ConvertThread(QThread):
    ...
    def __init__(self, ds_data, ratio_data, epoch_data, batch_data, output_data,
                 earlystopping_enabled, earlystopping_threshold, earlystopping_tolerance):
        ...
        config.set_para('keypoint', 'split_ratio', float(self.ratio_data))
        config.set_para('keypoint', 'epochs', self.epoch_data)
        config.set_para('keypoint', 'train_batch_size', self.batch_data)
        config.set_para('keypoint', 'output_path', self.output_data)
        config.set_para('keypoint', 'labelme_dataset_path', self.ds_data)
        #新增早停参数
        config.set_para('keypoint', 'earlystopping_enabled', self.earlystopping_enabled)
        config.set_para('keypoint', 'earlystopping_threshold', self.earlystopping_threshold)
        config.set_para('keypoint', 'earlystopping_tolerance', self.earlystopping_tolerance)

...
from PyQt5.QtGui import QIntValidator, QCursor, QDoubleValidator, QColor
from PyQt5.QtCore import Qt
from PyQt5.QtWidgets import QWidget
from qtpy import QtWidgets
from src.labelme.labelme.convert_model.classification.convert_thread import ConvertThread
from src.labelme.labelme.run_cmd_utils import is_contains_chinese, check_path
from src.labelme.labelme.convert_model.classification.component.classification import Ui_frame_body
...
class ConvertModel(QWidget, Ui_frame_body):
    ...
    def translate(self):
        self.label_title.setText(self.tr("Classification"))
    ...
    def outputMsg(self):
        ...
        self.output_path = QtWidgets.QFileDialog.getExistingDirectory(None, self.tr("Choose Folder"),
                                                                      self.last_out_path)
        if self.output_path:
    ...
    def pkg_start(self):
        if self.start_button.text() == self.tr('Transfer'):
            ...
            if self.ds_data and self.ratio_data and self.epoch_data and self.batch_data and self.output_data:
                if not os.path.exists(os.path.join(os.path.dirname(self.ds_data), 'flags.txt')):
                    self.notes = Notes(self.tr("ERROR"), self.tr("flags.txt is not exist!"), "ERROR")
                    self.notes.show()
                    return
...
if __name__ == '__main__':
    ...
    window = ConvertModel(model_no="分类模型")
    window.show()
    sys.exit(app.exec_())
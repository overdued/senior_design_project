...
"}")
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/icon/image/close.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        self.icon_button.setMinimumSize(QtCore.QSize(30, 0))
        self.icon_button.setMaximumSize(QtCore.QSize(30, 16777215))
        font = QtGui.QFont()
        font.setFamily("宋体")
        font.setPointSize(10)
        font.setBold(False)
        ...
        self.progressBar = QtWidgets.QProgressBar(self.frame_form)
...
"}")
        ...
        self.cancel.setMinimumSize(QtCore.QSize(90, 31))
...
"            }")
        ...
        self.yes.setMinimumSize(QtCore.QSize(90, 31))
        self.yes.setMaximumSize(QtCore.QSize(90, 31))
        font = QtGui.QFont()
        font.setFamily("HarmonyOS Sans SC Medium")
        font.setPointSize(10)
...
class Upgrade(QWidget, Ui_frame_body):
...
class UpgradeThread(QThread):
    ...
    def run(self) -> None:
        file_to_save = os.path.join(os.getcwd(), self.filename)
        response = requests.get(self.url, stream=True)
        with open(file_to_save, "wb") as f:
            i = 0
            for data in tqdm(response.iter_content(chunk_size=1024)):
                self.process_value.emit(int(i / self.size * 100))
                f.write(data)
                i += 1

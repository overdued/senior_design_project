...
def eval(ground_truth_json, detection_results_json):
    ...
    print("Final Evaluation:", dict(eval_results))
...
def main():
    ...
    infer_path = os.path.dirname(cur_path)
    output_path = os.path.join(infer_path, "output")
    
    infer_quant = args.infer_quant
    if infer_quant:
        model_path = os.path.join(output_path, "yolov5s_quant.om")
    else:
        model_path = os.path.join(output_path, "yolov5s_bs1.om")
    ...
    if args.visible:
        ...
        save_pred_dir = os.path.join(args.output_dir, 'visible_arg_img')
        os.makedirs(save_pred_dir, exist_ok=True)
    else:
        class_names = yaml_data['names']

    model = InferSession(args.device_id, model_path)

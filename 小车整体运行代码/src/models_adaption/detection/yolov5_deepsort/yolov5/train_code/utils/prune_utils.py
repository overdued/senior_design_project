...
def gather_bn_weights(module_list, prune_idx):
    size_list = [module_list[idx][1].weight.data.shape[0] if type(module_list[idx][1]).__name__ == 'BatchNorm2d' else
                 module_list[idx][0].weight.data.shape[0] for idx in prune_idx]

    bn_weights = torch.zeros(sum(size_list))
    index = 0
    for idx, size in zip(prune_idx, size_list):
        bn_weights[index:(index + size)] = module_list[idx][1].weight.data.abs().clone() if type(
            module_list[idx][1]).__name__ == 'BatchNorm2d' else module_list[idx][0].weight.data.abs().clone()
        index += size
...
def write_cfg(cfg_file, module_defs):
    # modify filters
    for index, i in enumerate(module_defs):
        if i.get('type') == 'yolo':
            nc = i.get('classes')
            module_defs[index - 1]['filters'] = (int(nc) + 5) * 3
...
def init_weights_from_loose_model(compact_model, loose_model, CBL_idx, Conv_idx, CBLidx2mask):
    for idx in CBL_idx:
        ...
        loose_CBL = loose_model.module_list[idx]
        out_channel_idx = np.argwhere(CBLidx2mask[idx])[:, 0].tolist()
        compact_bn, loose_bn = compact_CBL[1] if type(compact_CBL[1]).__name__ == 'BatchNorm2d' else compact_CBL[0], \
                               loose_CBL[1] if type(loose_CBL[1]).__name__ == 'BatchNorm2d' else loose_CBL[0]
        compact_bn.weight.data = loose_bn.weight.data[out_channel_idx].clone()
        compact_bn.bias.data = loose_bn.bias.data[out_channel_idx].clone()
        compact_bn.running_mean.data = loose_bn.running_mean.data[out_channel_idx].clone()
        compact_bn.running_var.data = loose_bn.running_var.data[out_channel_idx].clone()
        ...
        if isinstance(input_mask, list):
            ...
            in_channel_idx = np.argwhere(input_mask)[:, 0].tolist()
        if type(compact_CBL[0]).__name__ == 'BatchNorm2d':
        ...
        if model_def['type'] == 'convolutional':
            ...
            if len(from_layers) == 1:
                ...
                if pruned_model.module_defs[next_idx]['type'] == 'convolutional_noconv':
                    ...
                    next_conv2 = pruned_model.module_list[i + from_layers[1] if from_layers[1] < 0 else from_layers[1]][
                        0]

...
def init_seeds(seed=0):
    random.seed(seed)
    np.random.seed(seed)
    torch_utils.init_torch_seeds(seed=seed)
...
def ap_per_class_prune(tp, conf, pred_cls, target_cls):
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.25, merge=False, classes=None, agnostic=False):
...
def output_to_target(output, width, height):
    ...
    if isinstance(output[0], torch.Tensor):
        output = [i.cpu().numpy() for i in output]
...
def increment_dir(output_path, dir, comment=''):

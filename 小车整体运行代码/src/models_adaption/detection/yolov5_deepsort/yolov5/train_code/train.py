...
def train(hyp, tb_writer, opt, device):
    ...
    log_dir = os.path.join(opt.output_path, log_dir)
    wdir = str(Path(log_dir) / 'weights') + os.sep  # weights directory
    wdir = os.path.join(opt.output_path, wdir)
    ...
    # generate label txt in output dir
    with open(f'{opt.output_path}/names.txt', 'w', encoding='UTF-8') as txt_file:
        for i in range(len(data_dict.get('names'))):
            txt_file.write(data_dict.get('names')[i] + '\n')
...
def parse_args():
    ...
    parser.add_argument('--hyp', type=str, default='./models/hyp.scratch-low.yaml', help='hyp.yaml path (optional)')
    ...
    parser.add_argument('--output_path', required=True, type=str, default='', help='output path')
...
def main(opt):
    if not os.path.isfile(opt.data):
    ...
    if not opt.evolve:
        if opt.local_rank in [-1, 0]:
            ...
            tb_writer = SummaryWriter(log_dir=os.path.join(opt.output_path,
                                                           increment_dir(opt.output_path, 'runs/exp', opt.name)))
        else:
            tb_writer = None

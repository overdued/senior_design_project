...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='', help='sparse model weights')
    ...
    modelyolov5 = Model('models/yolov5s.yaml', nc=2).to(device)
    ...
    eval_model = lambda model:test(opt.cfg, data=opt.data,
        weights=opt.weights,
        batch_size=12,
         img_size=img_size,
         iou_thres=0.5,
    ...
    for idx in prune_idx:
        ...
        highest_thre.append(model.module_list[idx][1].weight.data.abs().max().item() if type(model.module_list[idx][1]).__name__ == 'BatchNorm2d' else model.module_list[idx][0].weight.data.abs().max().item())
    ...
    def prune_and_eval(model, sorted_bn, percent=.0):
        ...
        remain_num = 0
        for idx in prune_idx:
            bn_module = model_copy.module_list[idx][1] if type(model_copy.module_list[idx][1]).__name__ == 'BatchNorm2d' else model_copy.module_list[idx][0]
    ...
    def obtain_filters_mask(model, thre, CBL_idx, prune_idx):
        ...
        filters_mask = []
        for idx in CBL_idx:
            bn_module = model.module_list[idx][1] if type(model.module_list[idx][1]).__name__ == 'BatchNorm2d' else model.module_list[idx][0]
            if idx in prune_idx:

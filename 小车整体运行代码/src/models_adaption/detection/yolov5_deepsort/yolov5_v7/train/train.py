...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    best_epoch = 0
    early_stopping_best_fitness = 0
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        print(f"process_value={int((epoch) / (epochs - start_epoch) * 90 + 6)}")
        ...
        if RANK in {-1, 0}:
            ...
            if opt.early_stopping:
                if map50 > early_stopping_best_fitness:
                    early_stopping_best_fitness = map50
                    best_epoch = epoch

                if epoch - best_epoch > opt.early_stopping_tolerance:
                    print(f"INFO: Early stopping after {opt.early_stopping_tolerance} epochs without improvement.")
                    stop = True

                if early_stopping_best_fitness > opt.early_stopping_threshold:
                    print(f"INFO: Early stopping as mAP50 ({early_stopping_best_fitness:.3f}) exceeded threshold ({opt.early_stopping_threshold:.3f}).")
                    stop = True
...
def parse_opt(known=False):
    ...
    parser.add_argument('--save_dir', type=str, default='', help='output path, default under yolov5 project root dir')
...
if __name__ == "__main__":
    ...
    config.set_para('detection', 'train_output', train_output_path)
    config.yaml_dump(yaml_file)

    opt.data = yaml_data['yolo_dataset_yaml_path']
    opt.weights = os.path.join(train_output_path, weights + '.pt')
    opt.epochs = yaml_data['epochs']
    opt.batch_size = yaml_data['train_batch_size']
    opt.freeze = yaml_data['freeze_layers']
    ...
    opt.save_dir = train_output_path
    try:
        ret = main(opt)
        print(ret)

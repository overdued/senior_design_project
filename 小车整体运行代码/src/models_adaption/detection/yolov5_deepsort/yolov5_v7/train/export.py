...
if __name__ == "__main__":
    ...
    convert_output_path = os.path.join(yaml_data['output_path'], 'convert_output')
    weights = yaml_data['weights']
    weights_path = os.path.join(yaml_data['train_output'], 'weights', 'best.pt')
    infer_batch_size = yaml_data['infer_batch_size']

    opt.batch_size = infer_batch_size
    opt.weights = weights_path
    opt.dynamic = False
    opt.opset = 11
    main(opt)

    onnx_path = os.path.join(yaml_data['train_output'], 'weights', 'best.onnx')
    if not os.path.exists(onnx_path):
        print(onnx_path, 'not exists')
        exit(-1)
    ...
    print(f"process_value=97")
    print('INFO: export onnx success.')

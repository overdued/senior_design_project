...
parser.add_argument('--checkpoint',
                    ...
                    default='./fast_res50_256x192.pth',
                    required=False,
                    type=str)
...
def validate(m, cfg, opt, heatmap_to_coord, batch_size=20, num_workers=20):
    ...
    if opt.ppose_nms:
        ...
        write_json(final_result, opt.output_path, form='coco', for_eval=True, outputfile='validate_rcnn_kpt.json')
    else:
        if opt.oks_nms:
            from alphapose.utils.pPose_nms import oks_pose_nms
            kpt_json = oks_pose_nms(kpt_json)

        output_rcnn = os.path.join(opt.output_path, 'validate_rcnn_kpt.json')
        with open(output_rcnn, 'w') as fid:
            json.dump(kpt_json, fid)

    sysout = sys.stdout
    res = evaluate_mAP(output_rcnn, ann_type='keypoints', ann_file=os.path.join(cfg.DATASET.TEST.ROOT, cfg.DATASET.TEST.ANN), halpe=halpe)
    sys.stdout = sysout
    return res


def validate_gt(m, cfg, opt, heatmap_to_coord, batch_size=20, num_workers=20):
    ...
    sysout = sys.stdout
    output_gt = os.path.join(opt.output_path, 'validate_gt_kpt.json')
    with open(output_gt, 'w') as fid:
        json.dump(kpt_json, fid)
    res = evaluate_mAP(output_gt, ann_type='keypoints', ann_file=os.path.join(cfg.DATASET.VAL.ROOT, cfg.DATASET.VAL.ANN), halpe=halpe)
    sys.stdout = sysout
    return res
...
def main():  
    ...
    cfg.DATASET.TEST.DET_FILE = os.path.join(opt.input_path, "json/test_det_yolo.json")
    
    if not os.path.isfile(opt.checkpoint):
        return 'ERROR: not found pth file.'   
    ...
    with torch.no_grad():
        gt_AP = validate_gt(m, cfg, opt, heatmap_to_coord, opt.batch, opt.num_workers)
        detbox_AP = validate(m, cfg, opt, heatmap_to_coord, opt.batch, opt.num_workers)
    temp_path = os.path.join(torch.hub.get_dir(), "checkpoints")
    if os.path.exists(temp_path):
        shutil.rmtree(temp_path)
    print('##### gt box: {} mAP | det box: {} mAP #####'.format(gt_AP, detbox_AP))
    return "INFO:Validata success!"

...
# import pycocotools 
# 已修改，为了少安装 Microsoft C++ Build Tools，去掉了用这个包的一部分代码
# 安装 pycocotools 前，需要先安装 Microsoft C++ Build Tools，
# 可参考 https://zhuanlan.zhihu.com/p/471661231 中步骤，到其中所说的官网下载安装更加有保障
...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model', # 可调参，默认 weights/ssd300_mAP_77.43_v2.pth
                        default='weights/ssd300_mAP_77.43_v2.pth', type=str,
                        help='Trained state_dict file path to open. If "interrupt", this will open the interrupt file.')
    parser.add_argument('--top_k', default=5, type=int, # 可调参,默认5
    ...
    parser.add_argument('--image', default=None, type=str, # 可调参,默认 None
    ...
    parser.add_argument('--score_threshold', default=0, type=float, # 可调参,默认 0
                        help='Detections with a score under this threshold will not be considered. This currently only works in display mode.')
    parser.add_argument('--dataset', default='my_custom_dataset', type=str, # 可调参，默认None
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        colors = torch.cat([get_color(j, on_gpu=device).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0) # 已修改,默认 on_gpu=img_gpu.device.index
...
def main_func(trained_model,class_names,image_path,score_threshold,top_k):
    parse_args()

    if args.config is not None:
        set_cfg(args.config)

    args.trained_model = trained_model # 已修改,在这里使用用户参数
    args.image = image_path # 已修改,在这里使用用户参数
    args.score_threshold = score_threshold # 已修改,在这里使用用户参数
    args.top_k = top_k # 已修改,在这里使用用户参数
    cfg.dataset.class_names=class_names # 已修改,修改相应的config参数,注意要和 train.py 相同
...
if __name__ == '__main__':
    trained_model = 'weights/yolact_base_transfer_1_20.pth'
    class_names = ('person','bear','car')

    image_path = 'data/coco_format_data/train/images/000000078823.jpg' # 若为图像分割可视化,则传入图片路径'data/coco_format_data/train/images/000000078823.jpg'
    score_threshold = 0.15
    top_k = 2
    main_func(trained_model,class_names,image_path,score_threshold,top_k)
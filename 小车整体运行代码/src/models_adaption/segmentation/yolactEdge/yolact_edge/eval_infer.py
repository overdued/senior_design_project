...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model', default='weights/yolact_resnet50_6_20.pth', type=str)
    parser.add_argument('--trained_om_model', default='yolact_edge.om', type=str)
    parser.add_argument('--class_names', default="('bear','person','car')", type=str)
    parser.add_argument('--valid_images', default='data/images', type=str)
    parser.add_argument('--valid_info', default='data/val_annotation.json', type=str)
    parser.add_argument('--save_folder', default='infer_results/', type=str)

    parser.add_argument('--image', default=None, type=str,  # 'data/images/000000023272.jpg'
                        help='A path to an image to use for display.')
    parser.add_argument('--images', default=None, type=str,  # 'data/coco_small_data/train/images:out'
                        help='An input folder of images and output folder to save detected images. Should be in the format input->output.')
    parser.add_argument('--top_k', default=2, type=int,
                        help='Further restrict the number of predictions to parse')

    parser.add_argument('--dataset', default='my_custom_dataset', type=str,  # coco2017_dataset my_custom_dataset
    ...
    parser.add_argument('--disable_tensorrt', default=True, dest='disable_tensorrt', action='store_false',
...
def evalimage(net: Yolact, path: str, save_path: str = None, detections: Detections = None, image_id=None):
    ...
    model = Net(device_id=0, model_path=args.trained_om_model)
    res = model([batch.numpy()])
    outs = [torch.from_numpy(ele) for ele in res[:5]]
    proto_out = torch.from_numpy(res[-1])
    # common postprocess
...
def evaluate(net: Yolact, dataset, train_mode=False, train_cfg=None):
    ...
    if args.image is not None:
        ...
        evalimage(net, args.image, args.save_folder + os.sep + 'infer_segmentation_result.jpg', detections=detections,
                  image_id="0")
...
def print_maps(all_maps):
    ...
    with open(args.save_folder + os.sep + 'infer_mAP.txt', 'w') as f:
        f.write(output_str)

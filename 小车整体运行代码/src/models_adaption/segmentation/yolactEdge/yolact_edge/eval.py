...
def parse_args(argv=None):
    ...
    # 有两种方式验证 （现在只开放了 1.2）
    # 1. 给定dataset，得到 mAP
    #       1.1 开源模型 + coco validation 数据集
    #       1.2 迁移训练模型 + 自己的有标签数据集
    # 2. 给定图片，得到图片分割结果
    #       2.1 开源模型 + 图片 
    #       2.2 迁移训练模型 + 图片
    ...
    # 若给定图片，则需要指定 image 路径
    parser.add_argument('--image', default=None, type=str, # 'data/coco_small_data/train/images/000000186422.jpg'
    ...
    args.class_names = tuple(args.class_names.split(','))
...
def evaluate(net:Yolact, dataset, train_mode=False, train_cfg=None):
    ...
    if args.image is not None:
        ...
        # else:
        evalimage(net, args.image, args.save_folder+os.sep+'infer_segmentation_result.jpg', detections=detections, image_id="0")

        if args.output_coco_json:
            detections.dump()

        # return
        return 'INFO:Evaluate success!'

...
set_ip() {
  file=/etc/sysconfig/network-scripts/ifcfg-"$1"
	...
	if [[ $dhcp4 == "yes" ]]; then
	  { echo "BOOTPROTO=dhcp"; echo "DEFROUTE=no"; echo "NAME=$1"; echo "DEVICE=$1"; echo "ONBOOT=yes"; } >> "$file"
		echo "set $1 dhcp4 to yes."
	else
    { echo "BOOTPROTO=static"; echo "DEFROUTE=yes"; echo "NAME=$1"; echo "DEVICE=$1"; echo "ONBOOT=yes"; } >> "$file"
		if [[ $address == "" ]]; then
			eval address=\$"$1"_def_ip
		fi
		echo "IPADDR=$address" >> "$file"
		if [ "$mask" == "" ]; then
		  eval mask="$def_mask"
		fi
		echo "PREFIX=$mask" >> "$file"
		echo "set $1 ip=$address, mask=$mask"
	fi
	echo "set $1 ip finish."
	if [[ $route != "" && $route_flag == "false" ]]; then
		echo "GATEWAY=$route" >> "$file"
		route_flag=true
		echo "set $1 route=$route."
	fi
	# shellcheck disable=SC2086
  if [[ $dns_pre != "" ]]; then
		echo "DNS1=$dns_pre" >> "$file"
		echo "set $1 dns=$dns_pre."
	fi
	if [[ $dns_alter != "" ]]; then
		echo "DNS2=$dns_alter" >> "$file"

...
uint16 ServoPwmDuty[8] = {1500,2000,1500,1500,1500,1500,2500,1500};	//PWM脉冲宽度
...
uint16 ServoTime = 2000;			//舵机从当前角度运动到指定角度的时间，也就是控制速度，2000表示2ms


// 设置单个舵机转动的目标位置机转动时间的函数
void ServoSetPluseAndTime(uint8 id,uint16 p,uint16 time)
{
	if(id >= 0 && id <= 7 && p >= 500 && p <= 2500)
	{
		if(time < 20)            // 防止时间过短，不到20ms
			time = 20;
		if(time > 30000)         // 防止时间过长
			time = 30000;
		
		if(id == 6)              // 6号舵机，也就是底部的舵机 0-180°
		{
			if(p > 2500)
				p = 2500;
			else if(p < 500)
...
void ServoPwmDutyCompare(void)//脉宽变化比较及速度控制
{
	...
	if(ServoPwmDutyHaveChange)//停止运动并且脉宽发生变化时才进行计算      ServoRunning == FALSE && 
	{
		...
		for(i=0;i<8;i++)            // 循环是为了比较初始设置的脉宽与期望脉宽的差并得到每一个20ms周期的脉宽步进
		{
...
void InitPWM(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_8;            // server5  server6
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);

	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10| GPIO_Pin_11 | GPIO_Pin_12;  // server1  server2  server3
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;            // server4
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;

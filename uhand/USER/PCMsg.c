...
//	NVIC_InitTypeDef NVIC_InitStructure;
	...
	USART_InitStructure.USART_BaudRate = 115200;//一般设置为9600,试一试和atlas通信改为115200
...
void Uart1SendData(BYTE dat)   // BYTE 八位无符号数据
{
	// 状态寄存器的TC（Transmission complete）位
	while((USART1->SR&0X40)==0);//循环发送,直到发送完毕   发送完才会跳出循环
	USART1->DR = (u8) dat;
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)   // 接收到了数据准备读取
	{
			rxBuf = USART_ReceiveData(USART1);//(USART1->DR);	//读取接收到的数据
			if(!fFrameStart)   // 一开始是肯定能进入的
			{
				if(rxBuf == 0x55)  // 接收到的数据是U 两个作为帧头
				{

					startCodeSum++;
					if(startCodeSum == 2)   // 两个作为帧头
					{
						startCodeSum = 0;
						fFrameStart = TRUE;   // 转变为TRUE
						messageLength = 1;
					}
				}
				else
			...
			if(fFrameStart)    // 一旦该变量转变为TRUE，也就是接收到了两个帧头
			{
...
//			break;
	...
	dat[4] = prm1;
	dat[5] = prm2;
	UART1SendDataPacket(dat,datlLen + 2);   // 将数据包发送出去
}

static bool UartRxOK(void)
{
	if(fUartRxComplete)    // 接收完数据
	{
...
void TaskPCMsgHandle(void)
{
	...
	{
		LED = !LED;          // 翻转led
		cmd = UartRxBuffer[3];    // 提取命令
 		switch(cmd)
 		{
 			case CMD_MULT_SERVO_MOVE: // 3  多个舵机相同时间运动
				servoCount = UartRxBuffer[4];  // 需要控制的舵机个数
				time = UartRxBuffer[5] + (UartRxBuffer[6]<<8);  // 控制舵机时间
				for(i = 0; i < servoCount; i++)
				{
					id =  UartRxBuffer[7 + i * 3];    // 舵机id
					pos = UartRxBuffer[8 + i * 3] + (UartRxBuffer[9 + i * 3]<<8);  // 舵机位置
	
					ServoSetPluseAndTime(id,pos,time);               // 设置舵机转动与时间
					BusServoCtrl(id,SERVO_MOVE_TIME_WRITE,pos,time); // 总线控制舵机
				}			
        McuToPCSendData(CMD_MULT_SERVO_MOVE,0,0);				
 				break;
			
			case CMD_FULL_ACTION_RUN: // 6 
			...
			case CMD_FULL_ACTION_STOP:  // 7
				FullActStop();   // 停止动作组运行
				break;
				
			case CMD_FULL_ACTION_ERASE:  // 8 清除动作组
				FlashEraseAll(); 
				McuToPCSendData(CMD_FULL_ACTION_ERASE,0,0);
				break;

			case CMD_ACTION_DOWNLOAD:   // 25
